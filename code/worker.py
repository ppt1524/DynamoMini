...
class Worker(rpyc.Service):
    def __init__(self, port:int, redis_port:int) -> None:
        ...
        # self.visualization = RingVisualization(self.rds, self.hashmap)
        # viz_thread = threading.Thread(
        #     target=self.visualization.start_visualization,
        #     daemon=True
        # )
        # viz_thread.start()
    ...
    def exposed_get_nodes_status(self):
        """
        Returns the status of all nodes in the system
        Returns:
            Dict containing active and down nodes with their details
        """
        try:
            active_nodes = {}
            down_nodes = {}
            ...
            try:
                for node_hash, vc in self.routing_table.items():
                    active_nodes[node_hash] = {
                        'ip': vc.ip,
                        'port': vc.port,
                        'version': vc.version_number,
                        'load': vc.load,
                        'range': f"[{vc.start_of_range}, {node_hash}]"
                    }
            finally:
            ...
            try:
                for node_hash, vc in self.down_routing_table.items():
                    down_nodes[node_hash] = {
                        'ip': vc.ip,
                        'port': vc.port,
                        'version': vc.version_number,
                        'load': vc.load,
            ...
            logging.error(f"Error getting nodes status: {e}")
            return {
                "status": self.FAILURE,
                "error": str(e)
            }
            ...
            while max_retry > 0:
                try:
                    ...
                    if res['status'] == self.SUCCESS:
                        ...
                        keys_timestamp = pickle.loads(res['keys_timestamp'])
    ...
    def exposed_update_table(self, routing_info:Dict[Any, Any]):
        try:
            ...
            # Update node's own range
            self.start_of_range = routing_info['new_start']
    ...
    def handle_node_failure(self, failed_node_hash: str) -> None:
        """
        Updates ranges when a node fails. Should be called when a node failure is detected.
        Args:
            failed_node_hash: Hash of the failed node
        """
        try:
            logging.debug(f"Handling failure for node: {failed_node_hash}")
            
            # Get sorted list of active nodes
            active_nodes = sorted(self.routing_table.keys())
            if not active_nodes:
                logging.debug("No active nodes available to handle failure")
                return

            # Get failed node's info
            failed_node = self.down_routing_table.get(failed_node_hash)
            if not failed_node:
                logging.debug(f"Failed node {failed_node_hash} not found in down_routing_table")
                return

            # Find the next active node that should take over
            idx = bisect(active_nodes, failed_node_hash)
            next_node_hash = active_nodes[idx % len(active_nodes)]
            
            self.lock_routing_table.acquire()
            try:
                next_node = self.routing_table[next_node_hash]
                
                # Store old range for logging
                old_start = next_node.start_of_range
                
                # Update the range of the next node to include failed node's range
                next_node.start_of_range = failed_node.start_of_range
                next_node.version_number += 1
                
                logging.debug(f"Updated node {next_node_hash} range: {old_start} -> {next_node.start_of_range}")
                ...
                try:
                    ...
                    update_info = {
                        ...
                        "new_end": next_node_hash,
                        "new_added": {
                            "start_of_range": failed_node.start_of_range,
                            "end_of_range": next_node_hash,
                            "ip": next_node.ip,
                            "port": next_node.port,
                            "version_number": next_node.version_number,
                            "load": next_node.load
                        }
                    }
                    
                    conn.root.update_table(update_info)
                    logging.debug(f"Successfully notified node about range update")
                    
                except Exception as e:
                    logging.error(f"Failed to notify node about range update: {e}")
                    # Consider reverting the range update if notification fails
                    next_node.start_of_range = old_start
                    next_node.version_number -= 1
            ...
            logging.error(f"Error handling node failure: {e}")


    def handle_node_recovery(self, recovered_node_hash: str) -> None:
        """
        Updates ranges when a node recovers. Should be called when a down node comes back up.
        Args:
            recovered_node_hash: Hash of the recovered node
        """
        try:
            logging.debug(f"Handling recovery for node: {recovered_node_hash}")
            
            # Get the recovered node's info
            recovered_node = self.down_routing_table.get(recovered_node_hash)
            if not recovered_node:
                logging.debug(f"Recovered node {recovered_node_hash} not found in down_routing_table")
                return
                
            # Get sorted list of active nodes
            active_nodes = sorted(self.routing_table.keys())
            
            # Find the node that took over the failed node's range
            idx = bisect(active_nodes, recovered_node_hash)
            current_owner_hash = active_nodes[idx % len(active_nodes)]
            current_owner = self.routing_table[current_owner_hash]
            
            # Only update if the current owner's start range matches the recovered node's original range
            if current_owner.start_of_range == recovered_node.start_of_range:
                self.lock_routing_table.acquire()
                try:
                    # Restore the original ranges
                    current_owner.start_of_range = recovered_node_hash
                    current_owner.version_number += 1
                    ...
                    try:
                        ...
                        update_info = {
                            ...
                            "new_end": current_owner_hash,
                            "new_added": {
                                "start_of_range": recovered_node_hash,
                                "end_of_range": current_owner_hash,
                                "ip": current_owner.ip,
                                "port": current_owner.port,
                                "version_number": current_owner.version_number,
                                "load": current_owner.load
                            }
                        }
                        
                        conn.root.update_table(update_info)
                        logging.debug(f"Successfully notified node about range update after recovery")
                        
                    except Exception as e:
                        logging.error(f"Failed to notify node about range update after recovery: {e}")
                        
                finally:
                    self.lock_routing_table.release()
                    
        except Exception as e:
            logging.error(f"Error handling node recovery: {e}")    
        ...
        for node in to_ping:  
            if node in down_nodes:
                ...
                if response == True:
                    ...
                    self.handle_node_failure(str(node))
            elif node in nodes: 
    ...
    def print_routing_table(self):
        """
        Prints the current state of routing tables
        """
        # Create divider line
        divider = "--" * 10
        
        # Print active routing table
        logging.debug(f"{divider} Routing table of {self.port} {divider}")
        logging.debug(f"{divider} Active Routing table {divider}")
        
        for node, vc in self.routing_table.items():
            logging.debug(
                f"[{int(vc.start_of_range) % 1000}, {int(node) % 1000}] | "
                f"url = ({vc.ip}, {vc.port}), "
                f"version = {vc.version_number} | "
                f"Load = {vc.load}"
            )
        
        # Print down routing table
        logging.debug(f"{divider} Down Routing Table {divider}")
