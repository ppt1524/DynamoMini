...
class Client(rpyc.Service):
    ...
    def exposed_get_nodes_status(self):
        ...
        Returns:
            Dict containing formatted information about active and down nodes
        """
        try:
            # Try to get status from a random node
            max_retries = self.RETRIES
            while max_retries > 0:
                print("Here, in exposed nodes function: ")
                try:
                    # Pick a random node to query
                    node = random.choice(self.nodes)
                    who = random.randint(0, node["vnodes"] - 1)
                    url = (node["ip"], int(node["port"]) + who)
                    
                    conn = rpyc.connect(*url)
                    conn._config['sync_request_timeout'] = None
                    result = conn.root.get_nodes_status()
                    ...
                    if result["status"] == self.SUCCESS:
                        ...
                        # Format active nodes
                        for node_hash, info in result["active_nodes"].items():
                            node_info = (
                                f"Node {node_hash[-6:]}... at {info['ip']}:{info['port']}\n"
                                f"  Range: {info['range']}\n"
                                f"  Version: {info['version']}\n"
                                f"  Load: {info['load']}"
                            )
                            output["Active Nodes"].append(node_info)
                        
                        # Format down nodes
                        for node_hash, info in result["down_nodes"].items():
                            node_info = (
                                f"Node {node_hash[-6:]}... at {info['ip']}:{info['port']}\n"
                                f"  Range: {info['range']}\n"
                                f"  Version: {info['version']}\n"
                            ...
                            output["Down Nodes"].append(node_info)
                    ...
                    logging.debug(f"Failed to get status from node {url}: {e}")
                    max_retries -= 1
                    continue
            
            return {
                "status": self.FAILURE,
                "error": "Could not get node status after maximum retries"
            }
            
        except Exception as e:
            logging.error(f"Error in get_nodes_status: {e}")
            return {
                "status": self.FAILURE,
                "error": str(e)
            }

    def exposed_print_nodes_status(self):
        """
        Prints a formatted view of all nodes status
        """
        result = self.exposed_get_nodes_status()
        if result["status"] == self.SUCCESS:
            nodes = result["nodes"]
            
            print("\n=== Active Nodes ===")
            if nodes["Active Nodes"]:
                for node in nodes["Active Nodes"]:
                    print(f"\n{node}")
            else:
            ...
            print("\n=== Down Nodes ===")
            if nodes["Down Nodes"]:
                for node in nodes["Down Nodes"]:
                    print(f"\n{node}")
            else:
                print("No down nodes")
            print("\n")
        else:
            print(f"Error getting nodes status: {result.get('error', 'Unknown error')}")
    ...
    def get_routing_info(self, key):
        while True:
            try:
                ...
                print(f"mere naame is  routing info from {url}")
                conn = rpyc.connect(*url)
                conn._config['sync_request_timeout'] = None 
